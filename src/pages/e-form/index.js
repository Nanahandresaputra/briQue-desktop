...
const Eform = ({ outletCode }) => {
  const { formStructure, listForm, getServices, getEmail } = useSelector((state) => state.briQueReducer);
  ...
  useEffect(() => {
    // dispatch(BRIQUE_ACTION.formStructureAction(dataParams.name)).catch(({ errorMssg }) => {
