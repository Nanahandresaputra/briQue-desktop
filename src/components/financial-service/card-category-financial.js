...
const CardCategoryFinancial = ({ data, service, getServices, category }) => {
  ...
  let icons = [
    {
      ...
      icons:
        ...
        ) : data.name === "paymentBriva" || data.name === "setorTunaiWIC" || data.name === "pencairanCekBg" || data.name === "tarikTunai" || data.name === "setoranTunai" ? (
    ...
    },
    {
      type: "nonFinancial",
      icons:
        data.name === "registrasiNotifikasi" || data.name === "registrasiInternetBanking" ? (
          <MdPhoneAndroid className="text-6xl" />
        ) : data.name === "penggantianBukuTabungan" || data.name === "pencetakanBukuTabungan" || data.name === "pencetakanRekeningKoran" ? (
  ...
  let counter = getServices?.filter((datas) => datas.name === data.name)?.length ? getServices?.filter((datas) => datas.name === data.name)?.length : 0;
  ...
  const decrement = () => {
    ...
    sameValue ? dispatch(BRIQUE_ACTION.setGetServices(getServices.filter((datas) => datas.id != sameValue.id))) : dispatch(BRIQUE_ACTION.setGetServices(getServices));
    ...
    <section className="flex items-center space-x-3">
      <div className="px-3 bg-gray-50 rounded-lg shadow-2xl flex-1">
        <div className="flex items-center justify-between text-blue-700">
          ...
          <p className="text-xl font-semibold text-blue-900">{data.displayName}</p>
