...
const MultiForm = ({ outletCode }) => {
  ...
  return (
    <section className="h-full max-h-[550px]">
      ...
      <div className={`my-7 flex justify-center ${state.getServices?.length < 5 ? "h-full" : "h-auto"}`}>
        <div className="space-y-3 w-8/12">
          ...
          <div className="space-y-5">
            {state.getServices?.map((data, index) => (
              <CardMultiForm data={data} key={index} listForm={listForm} stateData={state}>
                {data.displayName}
              </CardMultiForm>
            ))}
          </div>
      ...
      <Footer btnDisabled={state.getServices?.length === listForm?.length ? false : true} onClick={submissionFunctions}>
        Selanjutnya
      </Footer>
    </section>
  );
